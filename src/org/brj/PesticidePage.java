...
public class PesticidePage extends JFrame implements  ActionListener {
		...
		JButton o = (JButton)ae.getSource();
		  if (o.getText().equalsIgnoreCase("SHOW COST")){
			  try{
				  ...
				  str=a.tf.getText();
				  System.out.println();
			        rs=stmt.executeQuery("select price from pesticidess where pestname='"+str+"'");
			          while(rs.next()){
			  ...
			  ActualReportFrame.showViewer();
	           for(String x:ar)
	           {
	        	   try{
	        	        ...
	        	        rs=stmt.executeQuery("select price from pesticides where pestname='"+x+"'");
	        	          while(rs.next()){
